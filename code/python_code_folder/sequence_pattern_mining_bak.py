...
def block_sequence_detect (folder_name):
	...
	"""
	care_block_list = []
	care_block_list.append("block_1")	
	care_block_list.append("block_2")	
	care_block_list.append("block_3")	
	care_block_list.append("block_4")	
	care_block_list.append("block_5")
	care_block_list.append("block_6")
	care_block_list.append("block_7")	
	care_block_list.append("block_8")	
	care_block_list.append("block_9")
	"""
...
#{{{
	...
	for block_report_file in file_list:
		...
		for sub_path_name in sub_path_list:
			...
			block_list_num = 0
			for block in block_file_list:
				succeed = 0
				block_list_num += 1
				path_block = sub_path[sub_path_index]
				if not sub_path_index == 0 and block == sub_path_start_block:
					#this is path not finished but find new start block, must judge if this path already covered 95%
					cover_ratio = float(sub_path_index) / float(len(sub_path))
					if cover_ratio > 0.95:
						print sub_path_name
						succeed = 1
					sub_path_index = 0
					path_block = sub_path[sub_path_index]
				if block == path_block:
					#print "match " + block + " " + str(block_list_num)
				...
				if succeed == 1:	
					if sub_path_name in total_block_report_list:
...
def sequence_file_test_1 (folder_name):
#{{{
	sub_path_list = sequence_list_read()
	#sub_path_support_time_list = sub_path_support_time_list_gen(sub_path_list)

	file_list = global_APIs.get_folder_file_list (folder_name)
	total_block_report_list = []
	block_belong_node_list = database_opt.read_block_belong_node_list()
	for block_report_file in file_list:
		print block_report_file
		real_file_name = global_APIs.get_real_file_name(block_report_file)
		#0_c0-0c1s2n1.txt_block_report.txt
		pattern = r'.*\_(.*)\.txt\_block\_report\.txt$'	
		matchobj = re.match (pattern, real_file_name)
		file_node_id = matchobj.group(1) 
		...
		test_sub_path_num = 0
		warning_num = 0
		at_least_once_sequence = 0
		for sub_path_name in sub_path_list:
			sub_path = sub_path_list[sub_path_name]
			can_test = 1
			for path_block in sub_path:
				if not file_node_id in block_belong_node_list[path_block]:
					can_test = 0
			if can_test == 0:
				continue
			...
			for block in block_file_list:
				...
				if sub_path_index == len(sub_path):
					sub_path_index = 0
					at_least_once_sequence += 1 
			if not sub_path_index == 0 or match_at_least_once == 0:
				# match at least once == 0: did not match
				#sub_path_index == 0: match but not reach the last
				warning_num += 1
		print "warning " + str(warning_num)
		print "at least once " + str(at_least_once_sequence)
#}}}
...
#status_matrix_read
#{{{
def sequence_list_store (sub_path_result, prefix = ""):
#{{{
	file_name = "sequence_tmp_report/sequence_report"
	if not prefix == "":
...
#}}}
def prior_matrix_store (prior_matrix):
#{{{
	count = 0
	file_name = "sequence_tmp_report/prior_matrix_"
	file_name += str(count)
	count += 1	
	file_name += ".txt"
	fl = open (file_name, "w")	
	for block in prior_matrix:
		...
		for sub_block in prior_matrix[block]:
			if sub_block == "happen_count" or sub_block == "last_block":
				continue
			tmp += sub_block + " " + str(prior_matrix[block][sub_block]["count"]) + " "
		fl.write (tmp)
		fl.write("\n")
	fl.close()
#}}}
def prior_matrix_read ():
#{{{
	count = 0
	file_name = "sequence_tmp_report/prior_matrix_"
	file_name += str(count)
	...
	return prior_matrix
#}}}
def status_matrix_store (total_status_matrix):
#{{{ 
	file_name = "sequence_tmp_report/status_matrix.txt"
	fl = open (file_name, "w")	
