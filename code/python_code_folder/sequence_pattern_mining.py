...
def block_sequence_detect (folder_name):
	...
	for i in range(0, 100):
		new_care_block_list.append(care_block_list[i])
	care_block_list = list(new_care_block_list)
	...
	#ARES	
	pure_count = 0
	for care_block in total_status_matrix:
		if total_status_matrix[care_block]["tangle"] == []:
			pure_count += 1	
	print "pure_count " + str(pure_count)
	#ARES	
...
def node_common_block_summary (folder_name):
	...
	for node_name in block_happen_node_list:
		if len(block_happen_node_list[node_name]) == 18:
